...
const HardCopyDetails: React.FC = () => {
  ...
  const { query } = router;
  const { selectedImage, selectedCategory, productName } = query;
  ...
  useEffect(() => {
    const fetchPhotographyServices = async () => {
      try {
        ...
        if (!response.ok) throw new Error('Failed to fetch photography services');
        const data = await response.json();
        setPhotographyServices(data);
      } catch (error) {
        console.error('Error fetching photography services:', error);
        setAddedItemMessage("Failed to load photography services. Please try again later.");
      }
    };
  ...
  const handleSubmit = () => {
    ...
    setAddedItemMessage("Item added to cart!");
    router.push('/cart');
  };
        ...
        <div className="max-w-6xl w-full bg-white shadow-lg rounded-lg p-4 sm:p-8 flex flex-col sm:flex-row">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {photographyServices.map((service) => (
              <div key={service.id} className="bg-white shadow-xl rounded-xl p-6">
                <img src={service.images?.[0]?.url} alt={service.name} className="w-full h-48 object-cover rounded-lg shadow-md" />
                <h3 className="text-xl font-semibold text-gray-800 mt-4 text-center">{service.name}</h3>
                ...
                <div className="space-y-4 bg-gray-50 p-4 rounded-lg">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange ```javascript
                      {(e) => setFrameThickness(e.target.value)}
                      className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                    >
